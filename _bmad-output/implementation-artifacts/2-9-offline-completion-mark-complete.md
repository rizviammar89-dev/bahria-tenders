...
- **⚠️ Device E2E = manual smoke:** the "Mark complete" tap + completed-state render need a device against LAN-IP Supabase. Reuse the 2.8 resident + awarded job (re-provision via `scripts/provision_user.ts` since `db reset` wiped accounts; then award a job, then mark complete). The transition + authz core is fully pgTAP-proven.
